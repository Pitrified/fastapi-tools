...
- **Key Patterns:** Modular architecture in `src/`; configuration in `src/fastapi_tools/config/`; testing in `tests/`.
- **Relevant Folders:**
  - `src/fastapi_tools/` – Main application code
  - `src/fastapi_tools/config/` – Configuration management
...
- Ensure `.env` file is not present in the root, must be in `~/cred/fastapi_tools/.env`.
- Secrets should not be committed.

## Related

- `src/fastapi_tools/config/fastapi_tools_paths.py`
